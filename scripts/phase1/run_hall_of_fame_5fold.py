...
def train_single_fold(fold_idx, X_train, y_train, X_val, y_val, X_val_scaled, y_val_scaled, 
    ...
    # Get final train MEE
    final_train_mee = history['train_mee'][-1] if history['train_mee'] else float('inf')
    
    print(f"Val MEE: {best_val_mee:.4f} (epoch {best_epoch}) | Train MEE: {final_train_mee:.4f} | {elapsed:.1f}s")
    
    return {
        'fold': fold_idx + 1,
        'best_val_mee': best_val_mee,
        'best_epoch': best_epoch,
        'final_train_mee': final_train_mee,
...
def save_results(cv_results, config):
    ...
    with open(RESULTS_FILE, 'w', newline='') as f:
        ...
        writer.writerow([
            ...
            'final_train_mee', 'n_samples_train', 'n_samples_val', 
